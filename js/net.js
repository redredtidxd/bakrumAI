    ...
    class MultiplayerManager {
        ...
        handleMessage(m) {
            ...
            if (kind === 'presence') {
                ...
                this.peers.set(pid, {
                    name: String(data.n),
                    lastSeen: Date.now(),
                    x: 0, y: 1.55, z: 0, yaw: 0,
                    tx: 0, ty: 1.55, tz: 0, tyaw: 0,
                    f: false
                });
                if (isNew) {
                    this.addRemotePlayer(pid, this.peers.get(pid));
                    if (this.snapshotDone && this.joined && !this.roomFull) {
                        this.onToast('👤 ' + this.peers.get(pid).name + ' entró en la sala');
                        this.updateHost();
                    }
                } else {
                    this.peers.get(pid).lastSeen = Date.now();
                }
                return;
            }
        ...
        update(dt, playerPos, yaw, pitch, flashlightOn, wallBoxes, entity) {
            ...
            if (this.connected && this.joined && !this.roomFull) {
                ...
                // Refrescar presencia cada 10 s (broker con TTL de retenidos)
                this.presenceTimer += dt;
                if (this.presenceTimer > 10) {
                    this.presenceTimer = 0;
                    this.publishPresence();
                }
                ...
                if (this.cleanTimer > 3) {
                    ...
                    for (const [pid, p] of [...this.peers]) {
                        if (now - p.lastSeen > 12000) this.removePeer(pid);
                    }
